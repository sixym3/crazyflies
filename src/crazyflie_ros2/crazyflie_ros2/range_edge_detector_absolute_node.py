# -*- coding: utf-8 -*-
"""
Range Edge Detector using Absolute Height Thresholds

Detects edges when the range sensor reading exceeds:
  flight_height + (box_height * modifier)

This node can be swapped with range_edge_detector_node.py by changing the node name in launch files.
"""
import math
import csv
...
class RangeEdgeDetectorAbsolute(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f'Absolute Edge Detector initialized: listening on {self.topic}')
        self.get_logger().info(
...
def main(args=None):
    ...
    node = RangeEdgeDetectorAbsolute()
    try:
        rclpy.spin(node)
